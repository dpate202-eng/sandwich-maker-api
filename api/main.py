...
def update_sandwich(sandwich_id: int, sandwich: schemas.SandwichUpdate, db: Session = Depends(get_db)):
    updated = sandwiches.update(db, sandwich, sandwich_id)
    if updated is None:
        raise HTTPException(status_code=404, detail="Sandwich not found")
    return updated
...
def delete_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    deleted = sandwiches.delete(db, sandwich_id)
    if deleted is None:
        raise HTTPException(status_code=404, detail="Sandwich not found")
    return deleted
...
def update_resource(resource_id: int, resource: schemas.ResourceUpdate, db: Session = Depends(get_db)):
    updated = resources.update(db, resource, resource_id)
    if updated is None:
        raise HTTPException(status_code=404, detail="Resource not found")
    return updated
...
def delete_resource(resource_id: int, db: Session = Depends(get_db)):
    deleted = resources.delete(db, resource_id)
    if deleted is None:
        raise HTTPException(status_code=404, detail="Resource not found")
    return deleted
...
def update_recipe(recipe_id: int, recipe: schemas.RecipeUpdate, db: Session = Depends(get_db)):
    updated = recipes.update(db, recipe, recipe_id)
    if updated is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return updated
...
def delete_recipe(recipe_id: int, db: Session = Depends(get_db)):
    deleted = recipes.delete(db, recipe_id)
    if deleted is None:
        raise HTTPException(status_code=404, detail="Recipe not found")
    return deleted
...
def read_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    detail = order_details.read_one(db, order_detail_id)
    if detail is None:
...
def update_order_detail(order_detail_id: int, order_detail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    updated = order_details.update(db, order_detail, order_detail_id)
    if updated is None:
        raise HTTPException(status_code=404, detail="Order Detail not found")
    return updated
...
def delete_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    deleted = order_details.delete(db, order_detail_id)
    if deleted is None:
        raise HTTPException(status_code=404, detail="Order Detail not found")
    return deleted